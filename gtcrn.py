...
class ERB(nn.Module):
    def __init__(self, erb_subband_1, erb_subband_2, nfft=512, high_lim=8000, fs=16000):
        # e.g. erb_subband = 65, 64
        super().__init__()
        erb_filters = self.erb_filter_banks(erb_subband_1, erb_subband_2, nfft, high_lim, fs)
        nfreqs = nfft//2 + 1
        self.erb_subband_1 = erb_subband_1
        self.erb_fc = nn.Linear(nfreqs-erb_subband_1, erb_subband_2, bias=False)  # (192, 64)
        self.ierb_fc = nn.Linear(erb_subband_2, nfreqs-erb_subband_1, bias=False)  # (64, 192)
    ...
    def erb_filter_banks(self, erb_subband_1, erb_subband_2, nfft=512, high_lim=8000, fs=16000):
        ...
        生成ERB等效矩形带宽滤波器组
        """
        low_lim = erb_subband_1/nfft * fs    # e.g. 65 / 512 * 16000 = 2031.25
        erb_low = self.hz2erb(low_lim)
        erb_high = self.hz2erb(high_lim)
        erb_points = np.linspace(erb_low, erb_high, erb_subband_2)  # 在erb刻度下从low到high生成erb_subband_2个频带
        bins = np.round(self.erb2hz(erb_points)/fs * nfft).astype(np.int32)  # erb下均分刻度在hz下索引，映射到FFT频率索引
        erb_filters = np.zeros([erb_subband_2, nfft // 2 + 1], dtype=np.float32)  # ([64, 257])
        # 第一个滤波器权重，权重都是线性分配，递减
        ...
        erb_filters = erb_filters[:, erb_subband_1:]    # (64, 257) --> (64, 192)
...
class SFE(nn.Module):
    """
    Subband Feature Extraction，子带特征提取
    C * T * F -->(unfold) C * T * kF --> (reshape) kC * T * F
    """
...
class TRA(nn.Module):
    ...
    def forward(self, x):
        """x: (B,C,T,F)"""
        zt = torch.mean(x.pow(2), dim=-1)  # (B,C,T), 在F维度全局平均池化了，算的就是不同时间下的能量
        at = self.att_gru(zt.transpose(1, 2))[0]  # (B, T, 2C)
        at = self.att_fc(at).transpose(1, 2)  # (B, C, T)
        at = self.att_act(at)  # sigmoid映射到[0, 1]
        At = at[..., None]  # (B,C,T,1)
...
class GRNN(nn.Module):
    ...
    def forward(self, x, h=None):
        ...
        h: (num_layers, B, hidden_size), 上个RNN隐藏层状态
        """
        if h == None:
...
class DPGRNN(nn.Module):
    ...
    def forward(self, x):
        """x: (B, C, T, F)"""
        # Intra RNN
        x = x.permute(0, 2, 3, 1)  # （B, C, T, F） --> (B,T,F,C)，e.g. (1, 16, 63, 33) -->(1, 63, 33, 16)
        ...
        # Inter RNN
        x = intra_out.permute(0, 2, 1, 3)  # (B, T, F, C) -- > (B,F,T,C)(1, 33, 63, 16)
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        en_outs = []  # 存阶段结果用于skip connection
...
class GTCRN(nn.Module):
    ...
    def forward(self, spec):
        ...
        spec: (B, F, T, 2)
        """
        spec_ref = spec  # (B,F,T,2) = (B, 257, T, 2)

        spec_real = spec[..., 0].permute(0, 2, 1)   # (B, F, T) --> (B, T, F) = (B, T, 257)
        spec_imag = spec[..., 1].permute(0, 2, 1)   # (B, F, T) --> (B, T, F)
        spec_mag = torch.sqrt(spec_real**2 + spec_imag**2 + 1e-12)  # (B, T, F）
        feat = torch.stack([spec_mag, spec_real, spec_imag], dim=1)  # (B, 3, T, 257)

        feat = self.erb.bm(feat)  # (B, 3, T, 129) F从257 --> 129
...
if __name__ == "__main__":
    ...
    flops, params = get_model_complexity_info(model, (257, 63, 2), as_strings=True, print_per_layer_stat=True, verbose=True)
    print(flops, params)

    """
    causality check，因果检验
    两信号前半部分一样，后部分不一样，看是否前面处理一样后头不同
    """
