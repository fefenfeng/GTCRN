...
config = toml.load('configs/cfg_train.toml')
...
class loss_wavmag(nn.Module):
    ...
    def forward(self, est_stft, clean_stft, alpha=10):
        ...
        loss_wav = torch.norm((estimated - clean), p=1) / clean.numel() * 100  # 波形MAE
        loss_mag = torch.norm(abs(est_stft) - abs(clean_stft), p=1) / clean_stft.numel() * 100   # MAE
...
class loss_hybrid(nn.Module):
    ...
    def forward(self, pred_stft, true_stft):
        ...
        pred_mag = torch.sqrt(pred_stft_real**2 + pred_stft_imag**2 + 1e-12)  # 添加1e-12小数，防止出现0
        ...
        y_true = torch.sum(y_true * y_pred, dim=-1, keepdim=True) * y_true / (torch.sum(torch.square(y_true), dim=-1, keepdim=True) + 1e-8)     # 1e-8防止分母为0
